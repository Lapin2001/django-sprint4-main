...
def post_with_another_category(
    mixer, user, published_location, published_category, another_category
):
...
def post_of_another_author(
    mixer, another_user, published_location, published_category
):
    """Пост другого автора"""
    from blog.models import Post
    return mixer.blend(
...
def posts_with_unpublished_category(
    mixer, user, published_location, unpublished_category
):
...
@pytest.fixture
def future_posts(mixer, user, published_location, published_category):
...
@pytest.fixture
def post_comment_context_form_item(mixer, user, published_location, published_category):
...
@pytest.fixture
def create_post_context_form_item(mixer, user):
